(*
  ...
  Modified by Hamed Nemati and Robert Künnemann:
    - Define type Kripke to be parametric in proposition type, instead of fixed to string
    - fix proofs accordlingly (TODO)
*)

theory Epistemic_Logic imports
...
datatype ('i, 'a) fm
  = FF ("\<^bold>\<bottom>")
  | Pro 'a
  | Dis \<open>('i,'a) fm\<close> \<open>('i,'a) fm\<close> (infixr "\<^bold>\<or>" 30)
  | Con \<open>('i,'a) fm\<close> \<open>('i,'a) fm\<close> (infixr "\<^bold>\<and>" 35)
  | Imp \<open>('i,'a) fm\<close> \<open>('i,'a) fm\<close> (infixr "\<^bold>\<longrightarrow>" 25)
  | K 'i \<open>('i,'a) fm\<close>
...
theorem generalization:
  assumes valid: \<open>\<forall>(M :: ('i, 's, 'a) kripke) s. M, s \<Turnstile> p\<close>
  shows \<open>(M :: ('i, 's, 'a) kripke), s \<Turnstile> K i p\<close>
proof -
...
primrec eval :: \<open>('a \<Rightarrow> bool) \<Rightarrow> (('i,'a) fm \<Rightarrow> bool) \<Rightarrow> ('i,'a) fm \<Rightarrow> bool\<close> where
...
inductive SystemK :: \<open>('i,'a) fm \<Rightarrow> bool\<close> ("\<turnstile>e _" [50] 50) where
  A1: \<open>tautology p \<Longrightarrow> \<turnstile>e p\<close>
| A2: \<open>\<turnstile>e (K i p \<^bold>\<and> K i (p \<^bold>\<longrightarrow> q) \<^bold>\<longrightarrow> K i q)\<close>
| R1: \<open>\<turnstile>e p \<Longrightarrow> \<turnstile>e (p \<^bold>\<longrightarrow> q) \<Longrightarrow> \<turnstile>e q\<close>
| R2: \<open>\<turnstile>e p \<Longrightarrow> \<turnstile>e K i p\<close>
...
theorem soundness: \<open>\<turnstile>e p \<Longrightarrow> M, s \<Turnstile> p\<close>
  by (induct p arbitrary: s rule: SystemK.induct) (simp_all add: tautology)

section \<open>Derived rules\<close>

lemma K_FFI: \<open>\<turnstile>e (p \<^bold>\<longrightarrow> (\<^bold>\<not> p) \<^bold>\<longrightarrow> \<^bold>\<bottom>)\<close>
  by (simp add: A1)
...
lemma K_imply_head: \<open>\<turnstile>e imply (p # ps) p\<close>
proof -
...
lemma K_imply_Cons:
  assumes \<open>\<turnstile>e imply ps q\<close>
  shows \<open>\<turnstile>e imply (p # ps) q\<close>
proof -
  have \<open>tautology (imply ps q \<^bold>\<longrightarrow> imply (p # ps) q)\<close>
    by simp
  then have \<open>\<turnstile>e (imply ps q \<^bold>\<longrightarrow> imply (p # ps) q)\<close>
    using A1 by blast
  then show ?thesis
    using assms R1 by blast
qed

lemma K_imply_member: \<open>p \<in> set ps \<Longrightarrow> \<turnstile>e imply ps p\<close>
proof (induct ps)
  case Nil
  then show ?case
    by simp
...
lemma K_right_mp:
  assumes \<open>\<turnstile>e imply ps p\<close> \<open>\<turnstile>e imply ps (p \<^bold>\<longrightarrow> q)\<close>
  shows \<open>\<turnstile>e imply ps q\<close>
proof -
  have \<open>tautology (imply ps p \<^bold>\<longrightarrow> imply ps (p \<^bold>\<longrightarrow> q) \<^bold>\<longrightarrow> imply ps q)\<close>
    by (induct ps) simp_all
  then have \<open>\<turnstile>e (imply ps p \<^bold>\<longrightarrow> imply ps (p \<^bold>\<longrightarrow> q) \<^bold>\<longrightarrow> imply ps q)\<close>
    using A1 by blast
  then show ?thesis
    using assms R1 by blast
qed
...
theorem K_imply_weaken:
  assumes \<open>\<turnstile>e imply ps q\<close> \<open>set ps \<subseteq> set ps'\<close>
  shows \<open>\<turnstile>e imply ps' q\<close>
proof -
  ...
  qed
  then have \<open>\<turnstile>e (imply ps q \<^bold>\<longrightarrow> imply ps' q)\<close>
    using A1 by blast
  then show ?thesis
    using \<open>\<turnstile>e imply ps q\<close> R1 by blast
qed
...
lemma K_ImpI:
  assumes \<open>\<turnstile>e imply (p # G) q\<close>
  shows \<open>\<turnstile>e imply G (p \<^bold>\<longrightarrow> q)\<close>
proof -
  have \<open>set (p # G) \<subseteq> set (G @ [p])\<close>
    by simp
  then have \<open>\<turnstile>e imply (G @ [p]) q\<close>
    using assms K_imply_weaken by blast
  then have \<open>\<turnstile>e imply G (imply [p] q)\<close>
    using imply_append by metis
  then show ?thesis
    by simp
qed

lemma cut: \<open>\<turnstile>e imply G p \<Longrightarrow> \<turnstile>e imply (p # G) q \<Longrightarrow> \<turnstile>e imply G q\<close>
  using K_ImpI K_right_mp by blast

lemma K_Boole: \<open>\<turnstile>e imply ((\<^bold>\<not> p) # G) \<^bold>\<bottom> \<Longrightarrow> \<turnstile>e imply G p\<close>
proof -
  assume \<open>\<turnstile>e imply ((\<^bold>\<not> p) # G) \<^bold>\<bottom>\<close>
  then have \<open>\<turnstile>e imply G (\<^bold>\<not> \<^bold>\<not> p)\<close>
  ...
  then have \<open>\<turnstile>e (imply G (\<^bold>\<not> \<^bold>\<not> p) \<^bold>\<longrightarrow> imply G p)\<close>
    using A1 by blast
...
lemma K_DisE:
  assumes \<open>\<turnstile>e imply (A # G) C\<close> \<open>\<turnstile>e imply (B # G) C\<close> \<open>\<turnstile>e imply G (A \<^bold>\<or> B)\<close>
  shows \<open>\<turnstile>e imply G C\<close>
proof -
  have \<open>tautology (imply (A # G) C \<^bold>\<longrightarrow> imply (B # G) C \<^bold>\<longrightarrow> imply G (A \<^bold>\<or> B) \<^bold>\<longrightarrow> imply G C)\<close>
    by (induct G) auto
  then have \<open>\<turnstile>e (imply (A # G) C \<^bold>\<longrightarrow> imply (B # G) C \<^bold>\<longrightarrow> imply G (A \<^bold>\<or> B) \<^bold>\<longrightarrow> imply G C)\<close>
    using A1 by blast
  then show ?thesis
    using assms R1 by blast
qed
...
lemma K_conjoin_imply:
  assumes \<open>\<turnstile>e (\<^bold>\<not> conjoin G (\<^bold>\<not> p))\<close>
  shows \<open>\<turnstile>e imply G p\<close>
proof -
  have \<open>tautology (\<^bold>\<not> conjoin G (\<^bold>\<not> p) \<^bold>\<longrightarrow> imply G p)\<close>
    by (induct G) simp_all
  then have \<open>\<turnstile>e (\<^bold>\<not> conjoin G (\<^bold>\<not> p) \<^bold>\<longrightarrow> imply G p)\<close>
    using A1 by blast
  then show ?thesis
    using assms R1 by blast
qed
...
lemma K_distrib_K_imp:
  assumes \<open>\<turnstile>e K i (imply G q)\<close>
  shows \<open>\<turnstile>e imply (map (K i) G) (K i q)\<close>
proof -
  have \<open>\<turnstile>e (K i (imply G q) \<^bold>\<longrightarrow> imply (map (K i) G) (K i q))\<close>
  proof (induct G)
    case Nil
    then show ?case
      by (simp add: A1)
  next
    case (Cons a G)
    have \<open>\<turnstile>e (K i a \<^bold>\<and> K i (imply (a # G) q) \<^bold>\<longrightarrow> K i (imply G q))\<close>
      by (simp add: A2)
    moreover have
      \<open>\<turnstile>e ((K i a \<^bold>\<and> K i (imply (a # G) q) \<^bold>\<longrightarrow> K i (imply G q)) \<^bold>\<longrightarrow>
      ...
      by (simp add: A1)
    ultimately have \<open>\<turnstile>e (K i a \<^bold>\<and> K i (imply (a # G) q) \<^bold>\<longrightarrow> imply (map (K i) G) (K i q))\<close>
      using Cons R1 by blast
    moreover have
      \<open>\<turnstile>e ((K i a \<^bold>\<and> K i (imply (a # G) q) \<^bold>\<longrightarrow> imply (map (K i) G) (K i q)) \<^bold>\<longrightarrow>
        (K i (imply (a # G) q) \<^bold>\<longrightarrow> K i a \<^bold>\<longrightarrow> imply (map (K i) G) (K i q)))\<close>
      by (simp add: A1)
    ultimately have \<open>\<turnstile>e (K i (imply (a # G) q) \<^bold>\<longrightarrow> K i a \<^bold>\<longrightarrow> imply (map (K i) G) (K i q))\<close>
      using R1 by blast
    then show ?case
      by simp
  qed
...
(* consistency of a set of set of facts, saying:
   - no contradiction in same set
   - double negations, and etc. are deconstructed in some other set
   - tautologies in some other set
   - no contradiction for knowledge either
 *)
definition consistency :: \<open>('i,'a) fm set set \<Rightarrow> bool\<close> where
...
definition close :: \<open>('i,'a) fm set set \<Rightarrow> ('i,'a) fm set set\<close> where
...
theorem close_consistency:
  fixes C :: \<open>('i,'a) fm set set\<close>
...
instantiation fm :: (countable,countable) countable begin
...
primrec extend :: \<open>('i,'a) fm set \<Rightarrow> ('i,'a) fm set set \<Rightarrow> (nat \<Rightarrow> ('i,'a) fm) \<Rightarrow> nat \<Rightarrow> ('i,'a) fm set\<close> where
...
definition Extend :: \<open>('i,'a) fm set \<Rightarrow> ('i,'a) fm set set \<Rightarrow> (nat \<Rightarrow> ('i,'a) fm) \<Rightarrow> ('i,'a) fm set\<close> where
...
theorem Extend_maximal:
  assumes \<open>\<forall>y :: ('i,'a) fm. \<exists>n. y = f n\<close> \<open>finite_char C\<close>
...
theorem K_consistency: \<open>consistency {set G | G. \<not> \<turnstile>e imply G \<^bold>\<bottom>}\<close>
  by sorry
(*
  unfolding consistency_def
proof (intro conjI ballI allI impI notI)
  fix S :: \<open>('i,'a) fm set\<close>
  assume \<open>S \<in> {set G | G. \<not> \<turnstile>e imply G \<^bold>\<bottom>}\<close> (is \<open>S \<in> ?C\<close>)
  then obtain G where *: \<open>S = set G\<close> and \<open>\<not> \<turnstile>e imply G \<^bold>\<bottom>\<close>
    by  blast

  { fix i
    assume \<open>Pro i \<in> S \<and> (\<^bold>\<not> Pro i) \<in> S\<close>
    then have \<open>\<turnstile>e imply G (Pro i)\<close> \<open>\<turnstile>e imply G (\<^bold>\<not> Pro i)\<close>
      using K_imply_member * by blast+
    then have \<open>\<turnstile>e imply G \<^bold>\<bottom>\<close>
      using K_FFI K_right_mp by blast
    then show False
      using \<open>\<not> \<turnstile>e imply G \<^bold>\<bottom>\<close> by blast }
  ...
  { fix Z
    assume \<open>(\<^bold>\<not> \<^bold>\<not> Z) \<in> S\<close>
    then have \<open>\<turnstile>e imply G (\<^bold>\<not> \<^bold>\<not> Z)\<close>
      using K_imply_member * by simp
    then have \<open>\<not> \<turnstile>e imply (Z # G) \<^bold>\<bottom>\<close>
      using K_ImpI K_right_mp \<open>\<not> \<turnstile>e imply G \<^bold>\<bottom>\<close> by blast
  ...
  { fix A B
    assume \<open>(A \<^bold>\<and> B) \<in> S\<close>
    then have \<open>\<turnstile>e imply G (A \<^bold>\<and> B)\<close>
      using K_imply_member * by simp
    moreover have \<open>\<turnstile>e ((A \<^bold>\<and> B) \<^bold>\<longrightarrow> A)\<close> \<open>\<turnstile>e ((A \<^bold>\<and> B) \<^bold>\<longrightarrow> B)\<close>
      using A1 by force+
    ultimately have \<open>\<turnstile>e imply G A\<close> \<open>\<turnstile>e imply G B\<close>
      using K_right_mp K_imply_head R1 by (metis imply.simps(2))+
    then have \<open>\<not> \<turnstile>e imply (A # B # G) \<^bold>\<bottom>\<close>
      using K_imply_Cons \<open>\<not> \<turnstile>e imply G \<^bold>\<bottom>\<close> cut by metis
  ...
  { fix A B
    assume \<open>(\<^bold>\<not> (A \<^bold>\<or> B)) \<in> S\<close>
    then have \<open>\<turnstile>e imply ((\<^bold>\<not> B) # G) (\<^bold>\<not> (A \<^bold>\<or> B))\<close> \<open>\<turnstile>e imply G (\<^bold>\<not> (A \<^bold>\<or> B))\<close>
      using * K_imply_Cons K_imply_member by blast+
    moreover have \<open>\<turnstile>e imply ((\<^bold>\<not> B) # G) (\<^bold>\<not> (A \<^bold>\<or> B) \<^bold>\<longrightarrow> \<^bold>\<not> A)\<close> \<open>\<turnstile>e imply G (\<^bold>\<not> (A \<^bold>\<or> B) \<^bold>\<longrightarrow> \<^bold>\<not> B)\<close>
      by (simp_all add: A1 tautology_imply)
    ultimately have \<open>\<turnstile>e imply ((\<^bold>\<not> B) # G) (\<^bold>\<not> A)\<close> \<open>\<turnstile>e imply G (\<^bold>\<not> B)\<close>
      using K_right_mp by blast+

    then have \<open>\<not> \<turnstile>e imply ((\<^bold>\<not> A) # (\<^bold>\<not> B) # G) \<^bold>\<bottom>\<close>
      using \<open>\<not> \<turnstile>e imply G \<^bold>\<bottom>\<close> cut by blast
  ...
  { fix A B
    assume \<open>(\<^bold>\<not> (A \<^bold>\<longrightarrow> B)) \<in> S\<close>
    then have \<open>\<turnstile>e imply ((\<^bold>\<not> B) # G) (\<^bold>\<not> (A \<^bold>\<longrightarrow> B))\<close> \<open>\<turnstile>e imply G (\<^bold>\<not> (A \<^bold>\<longrightarrow> B))\<close>
      using * K_imply_member K_imply_Cons by blast+
    moreover have
      \<open>\<turnstile>e imply ((\<^bold>\<not> B) # G) (\<^bold>\<not> (A \<^bold>\<longrightarrow> B) \<^bold>\<longrightarrow> A)\<close>
      \<open>\<turnstile>e imply G (\<^bold>\<not> (A \<^bold>\<longrightarrow> B) \<^bold>\<longrightarrow> \<^bold>\<not> B)\<close>
      by (simp_all add: A1 tautology_imply)
    ultimately have \<open>\<turnstile>e imply ((\<^bold>\<not> B) # G) A\<close> \<open>\<turnstile>e imply G (\<^bold>\<not> B)\<close>
      using K_right_mp by blast+

    then have \<open>\<not> \<turnstile>e imply (A # (\<^bold>\<not> B) # G) \<^bold>\<bottom>\<close>
      using \<open>\<not> \<turnstile>e imply G \<^bold>\<bottom>\<close> cut by blast
  ...
  { fix A B
    assume \<open>(A \<^bold>\<or> B) \<in> S\<close>
    then have \<open>\<turnstile>e imply G (A \<^bold>\<or> B)\<close>
      using * K_imply_member by simp

    { assume \<open>(\<forall>G'. set G' = S \<union> {A} \<longrightarrow> \<turnstile>e imply G' \<^bold>\<bottom>)\<close>
        and \<open>(\<forall>G'. set G' = S \<union> {B} \<longrightarrow> \<turnstile>e imply G' \<^bold>\<bottom>)\<close>
      then have \<open>\<turnstile>e imply (A # G) \<^bold>\<bottom>\<close> \<open>\<turnstile>e imply (B # G) \<^bold>\<bottom>\<close>
        using * by (metis Un_insert_right list.simps(15) sup_bot.right_neutral)+
      then have \<open>\<turnstile>e imply G \<^bold>\<bottom>\<close>
        using \<open>\<turnstile>e imply G (A \<^bold>\<or> B)\<close> K_DisE by blast
      then have False
        using \<open>\<not> \<turnstile>e imply G \<^bold>\<bottom>\<close> by blast }
  ...
  { fix A B
    assume \<open>(\<^bold>\<not> (A \<^bold>\<and> B)) \<in> S\<close>
    then have \<open>\<turnstile>e imply G (\<^bold>\<not> (A \<^bold>\<and> B))\<close>
      using * K_imply_member by blast
    moreover have \<open>\<turnstile>e imply G (\<^bold>\<not> (A \<^bold>\<and> B) \<^bold>\<longrightarrow> (\<^bold>\<not> A) \<^bold>\<or> (\<^bold>\<not> B))\<close>
      by (simp add: A1 tautology_imply)
    ultimately have \<open>\<turnstile>e imply G ((\<^bold>\<not> A) \<^bold>\<or> (\<^bold>\<not> B))\<close>
      using K_right_mp by blast

    { assume \<open>(\<forall>G'. set G' = S \<union> {\<^bold>\<not> A} \<longrightarrow> \<turnstile>e imply G' \<^bold>\<bottom>)\<close>
        and \<open>(\<forall>G'. set G' = S \<union> {\<^bold>\<not> B} \<longrightarrow> \<turnstile>e imply G' \<^bold>\<bottom>)\<close>
      then have \<open>\<turnstile>e imply ((\<^bold>\<not> A) # G) \<^bold>\<bottom>\<close> \<open>\<turnstile>e imply ((\<^bold>\<not> B) # G) \<^bold>\<bottom>\<close>
        using * by (metis Un_insert_right list.simps(15) sup_bot.right_neutral)+
      then have \<open>\<turnstile>e imply G \<^bold>\<bottom>\<close>
        using K_DisE \<open>\<turnstile>e imply G ((\<^bold>\<not> A) \<^bold>\<or> (\<^bold>\<not> B))\<close> by blast
      then have False
        using \<open>\<not> \<turnstile>e imply G \<^bold>\<bottom>\<close> by blast }
  ...
  { fix A B
    assume \<open>(A \<^bold>\<longrightarrow> B) \<in> S\<close>
    then have \<open>\<turnstile>e imply G (A \<^bold>\<longrightarrow> B)\<close>
      using * K_imply_member by blast
    moreover have \<open>\<turnstile>e imply G ((A \<^bold>\<longrightarrow> B) \<^bold>\<longrightarrow> (\<^bold>\<not> A) \<^bold>\<or> B)\<close>
      by (simp add: A1 tautology_imply)
    ultimately have \<open>\<turnstile>e imply G ((\<^bold>\<not> A) \<^bold>\<or> B)\<close>
      using K_right_mp by blast

    { assume \<open>(\<forall>G'. set G' = S \<union> {\<^bold>\<not> A} \<longrightarrow> \<turnstile>e imply G' \<^bold>\<bottom>)\<close>
        and \<open>(\<forall>G'. set G' = S \<union> {B} \<longrightarrow> \<turnstile>e imply G' \<^bold>\<bottom>)\<close>
      then have \<open>\<turnstile>e imply ((\<^bold>\<not> A) # G) \<^bold>\<bottom>\<close> \<open>\<turnstile>e imply (B # G) \<^bold>\<bottom>\<close>
        using * by (metis Un_insert_right list.simps(15) sup_bot.right_neutral)+
      then have \<open>\<turnstile>e imply G \<^bold>\<bottom>\<close>
        using K_DisE \<open>\<turnstile>e imply G ((\<^bold>\<not> A) \<^bold>\<or> B)\<close> by blast
      then have False
        using \<open>\<not> \<turnstile>e imply G \<^bold>\<bottom>\<close> by blast }
  ...
  { fix A :: \<open>'i fm\<close>
    assume \<open>tautology A\<close>
    then have \<open>\<turnstile>e imply G A\<close>
      using tautology_imply A1 by blast
    then have \<open>\<turnstile>e imply (A # G) \<^bold>\<bottom> \<Longrightarrow> \<turnstile>e imply G \<^bold>\<bottom>\<close>
      using cut by blast
  ...
  { fix A i
    assume \<open>K i A \<in> S \<and> (\<^bold>\<not> K i A) \<in> S\<close>
    then have \<open>\<turnstile>e imply G (K i A)\<close> \<open>\<turnstile>e imply G (\<^bold>\<not> K i A)\<close>
      using K_imply_member * by blast+
    then have \<open>\<turnstile>e imply G \<^bold>\<bottom>\<close>
      using K_FFI K_right_mp by blast
    then show False
...
theorem K_finite_consistency: \<open>consistency (mk_finite_char (close {set G | G. \<not> \<turnstile>e imply G \<^bold>\<bottom>}))\<close>
  using K_consistency finite_consistency close_closed close_consistency by blast

theorem K_concrete_finite_consistency:
  defines \<open>C \<equiv> mk_finite_char (close {set G | G. \<not> \<turnstile>e imply G \<^bold>\<bottom>})\<close>
  assumes \<open>set G \<in> C\<close>
  shows \<open>\<not> \<turnstile>e imply G \<^bold>\<bottom>\<close>
...
proof -
  ...
  proof (rule ccontr)
    ...
    then have \<open>V \<union> {p \<^bold>\<or> \<^bold>\<not> p} \<notin> C\<close>
      ...
      by (metis (no_types, opaque_lifting) insert_iff subset_iff sup.absorb_iff1 sup.cobounded1)
    then show False
      using assms unfolding consistency_def maximal_def
      by simp
  qed
...
lemma K_not_neg_in_consistency: \<open>\<not> \<turnstile>e (\<^bold>\<not> p) \<Longrightarrow> {p} \<in> {set G | G. \<not> \<turnstile>e imply G \<^bold>\<bottom>}\<close>
proof -
  assume \<open>\<not> \<turnstile>e (\<^bold>\<not> p)\<close>
...
lemma K_inconsistent_neg:
  ...
  shows \<open>\<turnstile>e (\<^bold>\<not> p)\<close>
  using assms unfolding C_def mk_finite_char_def close_def
...
lemma inconsistent_conjoin:
  ...
  shows \<open>\<turnstile>e (\<^bold>\<not> conjoin G p)\<close>
proof (rule ccontr)
  ...
  assume \<open>\<not> \<turnstile>e (\<^bold>\<not> conjoin G p)\<close>
...
theorem K_in_maximal:
  defines \<open>C \<equiv> mk_finite_char (close {set G | G. \<not> \<turnstile>e imply G \<^bold>\<bottom>})\<close>
  assumes \<open>\<turnstile>e p\<close> \<open>V \<in> C\<close> \<open>maximal V C\<close>
...
proof -
  ...
  have \<open>\<turnstile>e (p \<^bold>\<longrightarrow> \<^bold>\<not> \<^bold>\<not> p)\<close>
    by (simp add: A1)
  then have \<open>\<turnstile>e (\<^bold>\<not> \<^bold>\<not> p)\<close>
    using \<open>\<turnstile>e p\<close> R1 by blast
  ...
  proof (rule ccontr)
    ...
    then have \<open>\<not> \<turnstile>e (\<^bold>\<not> \<^bold>\<not> p)\<close>
      using C_def K_concrete_finite_consistency by fastforce
    then show False
      using \<open>\<turnstile>e (\<^bold>\<not> \<^bold>\<not> p)\<close> by blast
  qed
qed
...
theorem exists_maximal_superset:
  fixes C :: \<open>('i :: countable, 'a :: countable) fm set set\<close>
...
type_synonym ('i,'a) s_max = \<open>('i,'a) fm set\<close>

abbreviation pi :: \<open>('i,'a) s_max \<Rightarrow> 'a \<Rightarrow> bool\<close> where
  \<open>pi s i \<equiv> Pro i \<in> s\<close>

abbreviation partition :: \<open>('i,'a) fm set \<Rightarrow> 'i \<Rightarrow> ('i,'a) fm set\<close> where
  \<open>partition V i \<equiv> {p. K i p \<in> V}\<close>

abbreviation reach :: \<open>('i,'a) fm set set \<Rightarrow> 'i \<Rightarrow> ('i,'a) s_max \<Rightarrow> ('i,'a) s_max set\<close> where
  \<open>reach C i V \<equiv> {W. partition V i \<subseteq> W \<and> W \<in> C \<and> maximal W C}\<close>

theorem model_existence:
  fixes p :: \<open>('i :: countable, 'a :: countable) fm\<close>
  defines \<open>C \<equiv> mk_finite_char (close {set G | G. \<not> \<turnstile>e imply G \<^bold>\<bottom>})\<close>
  assumes \<open>V \<in> C\<close> \<open>maximal V C\<close>
  (* pi \<rightarrow> \<pi>
     reach C \<rightarrow> K
     V \<rightarrow> s (current world)
  *)
...
proof -
  ...
  next
    ...
    proof (intro allI impI)
      ...
      then have \<open>\<turnstile>e (\<^bold>\<not> conjoin L (\<^bold>\<not> p))\<close>
        using inconsistent_conjoin \<open>W \<union> {\<^bold>\<not> p} \<notin> C\<close> C_def by blast
      then have \<open>\<turnstile>e imply L p\<close>
        using K_conjoin_imply by blast
      then have \<open>\<turnstile>e K i (imply L p)\<close>
        using R2 by fast
      then have \<open>\<turnstile>e imply (map (K i) L) (K i p)\<close>
...
lemma imply_completeness:
  assumes valid: \<open>\<forall>(M :: ('i, ('i :: countable, 'a) fm set, 'a :: countable) kripke) s.
    list_all (\<lambda>q. M, s \<Turnstile> q) G \<longrightarrow> M, s \<Turnstile> p\<close>
  shows \<open>\<turnstile>e imply G p\<close>
proof (rule K_Boole, rule ccontr)
  assume \<open>\<not> \<turnstile>e imply ((\<^bold>\<not> p) # G) \<^bold>\<bottom>\<close>

  let ?S = \<open>set ((\<^bold>\<not> p) # G)\<close>
  let ?C = \<open>mk_finite_char (close {set G | G. \<not> \<turnstile>e imply G \<^bold>\<bottom>})\<close>
  ...
  have \<open>?S \<in> ?C\<close>
    using \<open>\<not> \<turnstile>e imply ((\<^bold>\<not> p) # G) \<^bold>\<bottom>\<close> close_closed close_subset finite_char_subset
    by fast
  then have \<open>?V \<in> ?C\<close>
    using Extend_in_C K_finite_consistency \<open>finite_char ?C\<close> by blast

  have \<open>\<forall>y :: ('i,'a) fm. \<exists>n. y = from_nat n\<close>
    by (metis from_nat_to_nat)
...
theorem completeness:
  assumes \<open>\<forall>(M :: ('i :: countable, ('i,'a) fm set, 'a :: countable) kripke) s. M, s \<Turnstile> p\<close>
  shows \<open>\<turnstile>e p\<close>
...
abbreviation \<open>valid p \<equiv> \<forall>(M :: (nat, (nat, nat) s_max, nat) kripke) s. M, s \<Turnstile> p\<close>

theorem main: \<open>valid p \<longleftrightarrow> \<turnstile>e p\<close>
proof
  assume \<open>valid p\<close>
  with completeness show \<open>\<turnstile>e p\<close>
    by blast
next
  assume \<open>\<turnstile>e p\<close>
...
proof
  assume \<open>valid p\<close>
  then have \<open>\<turnstile>e p\<close>
    unfolding main .
